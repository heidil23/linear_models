...
Consider candidate model.s Should we be fitting linear or non-linear
models? pwl = piecewise linear model
...
Piecewise linear model is linear, but has a break point (here at weight
= 7) It’s possible to test if the difference in slope is statistically
significant

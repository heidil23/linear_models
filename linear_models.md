...
lm is used for linear models glm is used for generalized linear models
...
Note that assumption of normality is violated based on the scatterplot
above, because there’s greater variability in price at higher star
ratings. But generally we see that as star rating goes up, prices go up
...
Use this code to add residuals to your data frame. It’s possible to add
residuals from several different models.
...
This plot again shows that distributions are skewed, though means are
probably centered around zero.
...
Let’s fit a logistic regression for the binary outcome glm automatically
treats it as a linear outcome
...
Add predictions from logistic model to the nyc\_airbnb data frame.
